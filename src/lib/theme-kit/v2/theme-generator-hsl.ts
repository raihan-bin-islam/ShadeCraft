...
function generateColorPalette(baseColor: HSL, harmony: ColorHarmony): HSL[] {
  switch (harmony) {
    ...
    case "monochromatic":
      return [baseColor, adjustSaturation(createShade(baseColor, 15), -10), adjustSaturation(createTint(baseColor, 20), 15)];
...
export function generateRandomTheme(): GeneratedTheme {
  ...
  const darkVars = {
    ...
    "secondary-foreground": hslToCss(ensureContrast(secondaryPair.foreground, createTint(secondaryPair.background, 15))),
    muted: darkBgs.muted,
  ...
  const sidebarColors = generateOklchSidebarColors(backgroundOklch, foregroundOklch, primaryOklch, accentOklch, borderOklch);
